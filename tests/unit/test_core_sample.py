...
test_cases = [
    ...
    pytest.param(
        [
            google.cloud.bigquery.SchemaField("coords", "RECORD", fields=[
    ...
    pytest.param(
        [
            google.cloud.bigquery.SchemaField("history", "TIMESTAMP", mode="REPEATED"), # 5 * 8
        ],
    ...
    pytest.param(
        [
            google.cloud.bigquery.SchemaField("addresses", "RECORD", mode="REPEATED", fields=[
                google.cloud.bigquery.SchemaField("street", "STRING"), # 1KIB
                google.cloud.bigquery.SchemaField("zip", "INT64"),     # 8
            ]),
        ],
        pandas_gbq.core.sample._ARRAY_LENGTH_ESTIMATE * (pandas_gbq.constants.BYTES_IN_KIB + 8),
        id="Repeated_Struct",
    ),
    pytest.param(
        [
    ...
    pytest.param(
        [
            ...
            google.cloud.bigquery.SchemaField("history", "TIMESTAMP", mode="REPEATED"), # 40
            google.cloud.bigquery.SchemaField("details", "RECORD", fields=[
...
def test_estimate_row_size_parametrized(schema: Sequence[google.cloud.bigquery.SchemaField], expected_size: int):
